...
export default class MainScene extends Phaser.Scene {
  ...
  preload() {
    ...
    this.load.image('bla', 'assets/sprites/bla.png')
  }
  ...
  create() {
    ...
    const codexObj = this.add
      .image(150, 50, 'journal')
      .setScale(0.25)
      .setInteractive()
    ...
    bagSprite.on('pointerup', function (pointer) {
      ...
      const currentImage = bagSprite.texture.key

      const altImage = currentImage === 'bag' ? 'bla' : 'bag'
      bagSprite.setTexture(altImage)
    })
