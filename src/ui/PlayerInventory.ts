...
export default class PlayerInventory extends Phaser.GameObjects.Container {
  ...
  constructor(scene: Phaser.Scene, x: number, y: number, width: number, height: number, rows: number, cols: number, slotSize: number, slotSpacing: number, pageWidth: number) {
    ...
    const bg = createRoundedRectWithAlpha(scene, -32, -32, bgWidth, bgHeight, 20, this.bgColor, this.alphaValue);
    ...
    for (let i = 0; i < this.rows; i++) {
      for (let j = 0; j < this.cols; j++) {
        ...
        const slot = createRoundedRectWithAlpha(scene, slotX, slotY, this.slotSize, this.slotSize, 5, this.slotColor, 0.5);
  ...
  addItem(slotIndex: number, item: JSON, image?: Phaser.GameObjects.Image): boolean {
    const slot = this.slots[slotIndex];

    slot.itemData = item;
    slot.itemImage = image;

    const itemIcon = this.scene.add.image(slot.x + slot.width / 2, slot.y + slot.height / 2, image.texture.key);
